...
export default function TwitterManager({ service }) {
    ...
    useEffect(() => {
        ...
        if (query.get('connected') === 'twitter') {
            ...
            <div className="mx-auto max-w-6xl">
                <header className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <button
                        ...
                        disabled={connecting}
                        className="px-8 py-4 rounded-2xl bg-gradient-to-r from-blue-400 to-sky-400 text-[var(--foreground)] font-black uppercase tracking-widest text-sm hover:shadow-[0_0_30px_rgba(56,189,248,0.4)] transition-all disabled:opacity-50"
                    >
                            ...
                            <form onSubmit={handleSchedule} className="space-y-6">
                                <div>
                                    ...
                                    >
                                        ...
                                        {accounts.map(acc => (
                                            <option key={acc._id} value={acc._id}>@{acc.username || acc.name}</option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    />
                                    <p className="text-[10px] text-[var(--foreground)]/30 mt-2 italic">Note: Twitter native media upload is complex and this visual preview currently places URLs. Advanced media upload needs specific Twitter endpoints.</p>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    className="w-full py-4 rounded-xl bg-white text-black font-black uppercase tracking-widest text-xs hover:bg-sky-400 hover:text-[var(--foreground)] transition-all shadow-lg active:scale-95"
                                >
                                    Schedule Post
                                </button>
                            </form>
                        ...
                        <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                            {accounts.map(acc => (
                                <div key={acc._id} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-5 py-3 shrink-0">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-[var(--foreground)]">@{acc.username}</p>
                                        ...
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-2">
                                                    <img src={post.twitterAccountId?.profilePicture} className="h-4 w-4 rounded-full" alt="" />
                                                    <span className="text-[10px] font-black text-[var(--foreground)]/40 uppercase tracking-widest">@{post.twitterAccountId?.username}</span>
                                                </div>
